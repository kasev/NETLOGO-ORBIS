...
to merge-cities
  ask wilcities [
     if any? orbcities in-radius radius-size [
       create-link-to max-one-of orbcities in-radius radius-size [rank]
       ]
     ]  ;;; each city from Wilson's dataset creates link with its nearest neighbor from the orbis dataset
  ask links [
      ask end2 [ ;;those from orbcities, who revealed to be closest to some wilcity
        set closest-wil-pop [pop] of max-one-of in-link-neighbors [pop] ; min-one-of wilcities [distance myself] ;;adopt the population from the nearest one from them
        set closest-wil-name [name] of max-one-of in-link-neighbors [pop] ;; the linked orbcity receives information about label of its closest wilcity
        set size sqrt (closest-wil-pop / 2000)
        set color 45
        ask max-one-of in-link-neighbors [pop] [hide-turtle]
        ]
       ]
end
...
769
106
yellow = orbis site with wilson pop\nwhite = unmatched wilson site\ngreen = orbis site matched with orbis anonyomous\nred = umatched orbis sites\n
11
9.9
1
