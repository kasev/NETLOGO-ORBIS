...
to go
  ask churches [
     ...
     set size size + (church-age / 1000)
     let hatching-chance 0
     set hatching-chance random (pop / 1000) ;;; probablity to sprout a disseminator
  ...
  ask disseminators with [in-city? = true] [
    ...
    let hatching-chance 0
    set hatching-chance random 100 ; while in a city, I can hatch a church with probability 1:100
    if hatching-chance = 1 [
      ifelse any? churches-here
        [
          ask churches-here [set size size + 1]
        ]
        [
        hatch-churches 1
        [set shape "square"
