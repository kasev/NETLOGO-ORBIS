...
;to setup
  ...
  set orbnetwork-dataset gis:load-dataset "data/orbnetwork.shp"
...
to generate-routes
  ...
  ask roads [set color 56]
 ; foreach gis:feature-list-of orbnetwork-dataset
  ;    [ ask patches gis:intersecting orbnetwork-dataset
 ;       [ carefully
  ;         [ set cost gis:property-value orbnetwork-dataset "cost"  ]
   ;        [ set cost "" ] ] ]

;  foreach gis:feature-list-of orbnetwork-dataset [ [vector-feature] ->
;    if gis:intersects? vector-feature rivers
;    [ gis:draw vector-feature 1 ] ]
  foreach gis:find-greater-than orbnetwork-dataset "type" "upstream" [ [vector-feature] ->
...
to get-the-network
  ask ids with [count my-roads = 1] [
    create-roads-with min-n-of 2 other ids [distance myself]
    ]
  ask orbwilsites [
    create-road-with min-one-of ids [distance myself]
  ]

  ask ids with [pcolor != 31] [ask my-links [set color white]]

;  foreach gis:feature-list-of rivers-dataset [ [vector-feature] ->
;    if gis:intersects? vector-feature united-states
;    [ gis:draw vector-feature 1 ] ]
;  if gis:intersects? road provinces-dataset
;    [set color green]
  ;;ask roads with [set color 56]
