...
breed [ids id ] ; all nodes in the network, including cities: ids with [pop > 0]
...
to go
  ask churches [
     ...
     ; the churches can become bigger by age: set size size + (church-age / 10000)
     let hatching-chance 0
     set hatching-chance random 10 ;;; probablity to sprout a disseminator
    ...
    ask disseminators with [in-city? = false and travelled-distance < max-distance] [
      ...
      move-to my-new-id
      ; set my-last-id my-actual-id
      set my-actual-id my-new-id
      ;;set travelled-route route-to-take
      set travelled-distance travelled-distance + [link-length] of route-to-take
...
;if any? disseminators with [in-city? = false and travelled-distance < max-distance] [
;    ask disseminators with [in-city? = false and travelled-distance < max-distance] [
;      set route-to-take one-of [routes] of my-actual-id with [link-neighbors != my-last-id]  ;with [other-end != my-last-id]  ; with [color != red]
;      set my-new-id [[other-end] of end1] of route-to-take
;      move-to my-new-id
;      set my-last-id my-actual-id
;      set my-actual-id my-new-id
;      set travelled-route route-to-take
...
;  ]
  ...
  ask disseminators with [in-city? = true] [
    set city-stay-length ([pop] of my-actual-id / 1000) ; how long it is appropriate to stay here
    set length-of-stay (length-of-stay + 1) ;how long has been the disseminator here so far
    let hatching-chance 0
    set hatching-chance random 1 ; while in a city, I can hatch a church with probability 1:10
    if hatching-chance = 1 [
      ifelse any? churches-here
        [
          ask churches-here [set size (size) ]; * 1.1)]
        ]
        [
        hatch-churches 1
        [set shape "square"
