...
breed [ids id ]
breed [orbwilsites orbwilsite]
breed [provinces province]
undirected-link-breed [roads road]
undirected-link-breed [connections connection]
undirected-link-breed [helpings helping]

patches-own [cost my-province]
orbwilsites-own [name pop]
ids-own [my-type my-orb-id my-e pop name]
roads-own [
  my-end-type
  my-end-orb-id
  my-end-e
...
to setup-environment
  ...
  set orbwilsites-dataset gis:load-dataset "data/orbwilsites1.shp"
  set provinces-dataset gis:load-dataset "data/provinces.shp"
  set connections-dataset gis:load-dataset "data/supportive-roads.shp"
  gis:set-world-envelope (gis:envelope-union-of (gis:envelope-of orbnetwork-dataset)
                                                (gis:envelope-of orbwilsites-dataset)
                                                (gis:envelope-of provinces-dataset)
                                                )

   display-orbis-lines
   generate-orbwilsites
   constrain-by-provinces
   generate-connections-as-roads
...
to display-orbis-lines
      ...
      foreach but-last vertex [ [point] ->
        ...
        [ create-ids 1
          [ set xcor item 0 location
            ...
            [ create-road-with previous-turtle [
                set color 56
  ...
  ask roads with [my-end-type = "road" or my-end-type = "hires" or my-end-type = "ferry"] [set my-ratio-distance link-length * 52] ; * road-ratio)]
  ask roads with [my-end-type = "overseas" or my-end-type = "slowover" or my-end-type = "coastal"] [set my-ratio-distance link-length * 1] ;* sea-ratio]
  ask roads with [my-end-type = "upstream"] [set my-ratio-distance link-length * 7.5] ; * river-ratio]
end
...
to generate-connections-as-roads ;; used to produce the connections from the uncomplete network; instead of generated over and over, these are uploaded from outside
  ask ids with [pop < 1 and count my-links = 0] [die]
  ask ids with [pop < 1 and count my-links < 2] [
    create-roads-with min-n-of 2 other ids [distance myself]
    ]
  ask ids with [pop > 0] [
    create-roads-with min-n-of 1 other ids with [xcor != [xcor] of myself and ycor != [ycor] of myself] [distance myself]
  ]
  ask roads with [color != 56] ;set my-ratio-distance for just created connections
    [set my-ratio-distance (link-length * 52)]
end

to go

  ;if all? ids with [color = yellow] [stop]                                           ;;; Stops the simulation after the diffusion will reach all cities.
  ...
  ask ids with [pop > 0 and color = green]                  ;;; the diffusion can start just from cities which are already "infected"
    [
      ...
      ask out-link-neighbors with [pop > 0] [set color green]
    ]
      ;[set my-weighted-distance nw:weighted-distance-to other-end my-ratio-distance] ]
   ;;ask one-of ids with-min nw:weighted-distance-to [my-ratio-distance of myself] [ ; actual-asked-number-of-out-links my-out-links [(((summer-cost-kg + winter-cost-kg) / 2) * 100 / other-end-pop-size)] [
                                                     ;;; from the li st of randomly selected links to cities, the link with minimal costs and maximal recipient population size is chosen.
     ;    set color yellow                            ;;; the link is now yellow
     ;    ask other-end [set color yellow]]           ;;; the city is now yellow
...
to setup-simulation
  ask ids with [pop > 0] [set color yellow]
  reset-ticks
end
