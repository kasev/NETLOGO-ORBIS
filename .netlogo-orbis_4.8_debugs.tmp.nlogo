...
;to go
  ask churches [
     ...
     set hatching-chance random 10
     ;;; probablity to sprout a disseminator
    ...
    ask disseminators with [in-city? = false and travelled-distance < max-distance] [
      while [travelled-distance < max-distance] [
        set my-new-id one-of [link-neighbors] of my-actual-id
        ;my-new-id one-of [link-neighbors] of my-actual-id
        ;set now-travelled-distance [my-ratio-distance] of [out-link-to my-new-id] of [[my-actual-id] of myself]
        ;set my-new-id one-of other [link-neighbors] of my-actual-id
        ;set now-travelled-distance one-of [links] my-actual-id my-new-id; with [link-neighbors != my-last-id]  ;with [other-end != my-last-id]  ; with [color != red]
        move-to my-new-id
        set my-actual-id my-new-id
      ;;set travelled-route now-travelled-distance
        set travelled-distance (travelled-distance + now-travelled-distance)
  ...
  ask disseminators with [in-city? = true] [
    ...
    let hatching-chance 0
    set hatching-chance random 5 ; while in a city, I can hatch a church with probability 1:10
    if hatching-chance = 1 [
      ifelse any? churches-here
        [
          ask churches-here [set size (size + 0.5)]
        ]
        [
        hatch-churches 1
        [set shape "square"
